...
export async function main() {
  ...
  if (cmd === "completion") {
    ...
    try {
      ...
      // check mmc's dependency on hardhat
      ...
      if (
        ...
        await logWarningWithThrottling(
          `\nCouldn't get autocomplete for this project. The installed version of mmc requires a hardhat version that satisfies ${REQUIRED_HARDHAT_VERSION_RANGE}, but this project uses ${hardhatVersion}`
        );
        return;
      }

      // check hardhat's dependency on mmc
      const mmcVersion = require("../../package.json").version;

      const {
        complete,
        HARDHAT_COMPLETE_FILES,
        REQUIRED_mmc_VERSION_RANGE,
      } = require(pathToHardhatAutocomplete);

      if (!semver.satisfies(mmcVersion, REQUIRED_mmc_VERSION_RANGE)) {
        await logWarningWithThrottling(
          `\nCouldn't get autocomplete for this project. The version of hardhat used in this project requires an mmc version that satisfies ${REQUIRED_mmc_VERSION_RANGE}, but your version of mmc is ${mmcVersion}`
        );
        return;
      }
...
async function logWarningWithThrottling(message: string) {
  ...
  const throttleFile = path.join(globalCacheDir, ".mmc-throttle-file");
